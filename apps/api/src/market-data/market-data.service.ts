...
export class MarketDataService {
  ...
  async getAssetPrices(
    ...
    // Step 3: Try Alpha Vantage for remaining symbols (if API key available)
    if (remainingSymbols.length > 0 && hasAlphaVantageKey) {
      try {
        const alphaVantageResults = await this.fetchFromAlphaVantage(
          remainingSymbols,
          targetCurrency,
        );
        ...
        alphaVantageResults.forEach((result) => {
          foundAssets['Alpha Vantage'].push(result.symbol);
        });

        if (shouldLog) {
          this.logger.log(`Alpha Vantage found ${alphaVantageResults.length} assets`);
        }
      } catch (error) {
        this.logger.debug(`Alpha Vantage batch request failed: ${error.message}`);
      }
    }
